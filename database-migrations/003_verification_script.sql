...
BEGIN
    ...
    SELECT 
        ...
        twi.description,
        twi.distance_km,
        twi.estimated_budget,
        twi.tags,
        twi.available_spots,
        twi.max_participants,
        twi.current_participants,
        twi.start_date,
        twi.end_date,
        twi.region,
        twi.duration_days,
        twi.budget_per_person,
        twi.relevance_score,
        twi.primary_location_name,
        twi.primary_location_address,
        twi.images
    FROM trips_with_images twi
    ORDER BY 
        twi.relevance_score DESC,
        CASE 
            WHEN point_found THEN twi.distance_km 
            ELSE 999999 -- Put non-location searches after distance-based results
        END ASC NULLS LAST,
        twi.start_date ASC NULLS LAST
