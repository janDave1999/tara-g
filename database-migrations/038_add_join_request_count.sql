-- =====================================================
-- 038: Add join_request_count to get_user_owned_trips
-- =====================================================
-- Add a count of pending join requests to owned trips
-- so trip owners can see a badge on trip cards
-- =====================================================

DROP FUNCTION IF EXISTS get_user_owned_trips(UUID, TEXT, TEXT, INTEGER, INTEGER);

CREATE OR REPLACE FUNCTION public.get_user_owned_trips(
    p_user_id  UUID,
    ...
    SELECT
        ...
        (SELECT COUNT(*)
          ...
          WHERE tm2.trip_id = t.trip_id
            AND tm2.member_status = 'pending')     AS join_request_count
    FROM trips t
...
GRANT EXECUTE ON FUNCTION public.get_user_owned_trips TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_user_owned_trips TO anon;
