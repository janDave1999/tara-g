-- =====================================================
-- 048: Exclude already-joined/pending/invited trips from discover
-- =====================================================
-- get_discover_trips previously only excluded trips owned by
-- the user (owner_id != p_user_id). Trips the user had joined,
-- requested to join, or been invited to still appeared.
--
-- This migration adds two NOT EXISTS guards to both query paths:
--   1. trip_members  — excludes joined & pending requests
--   2. trip_invitations — excludes pending invitations
-- =====================================================

DROP FUNCTION IF EXISTS get_discover_trips(UUID, TEXT, TEXT, TEXT, TEXT[], INTEGER, INTEGER);
...
BEGIN
    ...
    IF v_has_preferences AND (v_budget_range IS NOT NULL OR v_travel_styles IS NOT NULL OR v_pace_pref IS NOT NULL) THEN
        ...
        WHERE t.status         = 'active'
          ...
          -- Exclude trips user is already a member of (joined or pending)
          AND NOT EXISTS (
          ...
          -- Exclude trips user has a pending invitation to
          AND NOT EXISTS (
              SELECT 1 FROM trip_invitations inv
              WHERE inv.trip_id   = t.trip_id
                AND inv.invitee_id = p_user_id
          ...
          -- Style filter: trip tags must overlap with any of the selected tags
          AND (p_travel_style IS NULL OR td.tags && p_travel_style)
        ORDER BY
            CASE
...
SELECT 'Updated get_discover_trips: exclude trips user has joined, requested, or been invited to' AS status;
